...
public class Card : MonoBehaviour
{
    ...
    {
        if (def.face == "") return; // No need to run if this isn’t a face card
    ...
    /// <summary>
    /// Gather all Sprite Renderers on this and its children into an array.
    /// </summary>
    ...
    /// </summary>
    /// <param name="s0rd">The sortingOrder for the face of the Card</param>
    public void SetSortingOrder(int s0rd)
    {
        ...
        foreach (SpriteRenderer srend in spriteRenderers)
        {
            if (srend.gameObject == this.gameObject)
            {
                ...
                srend.sortingOrder = s0rd; // Set its order to s0rd
